...
class Login extends Base
{
    ...
    public function dclogin()
    {
        ...
        if (!empty($res_data['access_token'])) {
            $user_dc = M('daichong_user')->where(['id' => 1])->find();
            if ($user_dc) {
                ...
                M('daichong_user')->insert(['user' => $user, 'pwd' => $pwd, 'token' => $res_data['access_token'], 'time' => time(), ]);
            }
...
//{"ret":0,"msg":"SUCCESS","data":"b18e8b3baaf4450ce30a70207e279376"}
        ...
        if (!empty($rsc)) {
            ...
            if ($user_dc) {
                ...
                M('daichong_user')->insert(['user' => $user, 'pwd' => $pwd, 'token' => $resd['data'], 'time' => time(),'yunying'=>$yunying ]);
            }
