...
class CustomerHezuoPrice extends Model
{
    ...
    public static function saveValue($customer_id, $product_id, $data)
    {
        $price = M('customer_hezuo_price')->where(array('product_id' => $product_id, 'customer_id' => $customer_id))->find();
        if (!$price) {
            $id = M('customer_hezuo_price')->insertGetId(array('customer_id' => $customer_id, 'product_id' => $product_id));
        }else{
            $id = $price['id'];
