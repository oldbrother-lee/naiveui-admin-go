...
//        return $model->create_wxpay_js($option, $config);
    ...
    {
        $porders = M('porder p')->join('reapi r', 'r.id=p.api_cur_id')->where(array('r.is_self_check' => 1,             'p.status' => 3))->order('p.pegging_time asc ,p.create_time asc')->limit(1)->field('p.id,p.mobile,p               .order_number,p.api_order_number,p.api_trade_num,p.api_cur_param_id,p.api_cur_id')->select();
