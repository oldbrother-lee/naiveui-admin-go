...
class Porder extends Admin
{
	...
	public function index()
	{
	   // echo("222");exit();
	    if($prefix = I('prefix')){
	        Cache::set('order_sn_prefix', $prefix); 
	    }
		$map = $this->create_map();
		if (I('sort')) {
			$sort = I('sort');
		...
		foreach ($list as $key => $val){
		    if(array_key_exists('order_sn', $val) && !$val['order_sn']){
		        $val['order_sn'] = $this->getOrderNo($val);
		    }
		}
		...
		$this->assign('order_sn_prefix',Cache::get('order_sn_prefix'));
		return view();
	}
