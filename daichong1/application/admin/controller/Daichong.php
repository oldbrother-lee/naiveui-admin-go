...
class Daichong extends Admin
{
    ...
    public function mian()
    {
        ...
        $user = M('daichong_user_xitu')->find();
    ...
    public function moneyset()
    {
        ...
        M('daichong_user_xitu')->where(['id' => 1])->update([$tb => $up]);
...
//Use of undefined constant upload - assumed 'upload' (this will throw an Error in a future version of PHP)"
    ...
    public function stop()
    {
        $user = get_dc_user();
        $user['type'] = 0;
        M('daichong_user_xitu')->where(['id' => $user['id']])->update($user);
        $header = [
            'Authorization:Bearer ' . $user['token'],
        ];
    ...
    public function uploadTokenWork($times = 0, $open = 0){
        if($times){
            $userlist = Cache::set('upload_token_log_times', $times);
        }
        if($open){
            $userlist = Cache::set('upload_token_log_open', 1);
        }else{
            $userlist = Cache::set('upload_token_log_open', 0);
        }
        $this->assign('open', Cache::get('upload_token_log_open'));
        $lists = M('upload_token_log')->order('id desc')->paginate(20);
        $page = $lists->render();
        $list = $lists->all();
//        foreach ($list as $k => $v) {
//
//        }
        $this->assign('times', Cache::get('upload_token_log_times'));
        $this->assign('page', $page);
        $this->assign('list', $list);
        return view();
    }
    public function delToken(){
        M('upload_token_log')->where('id','>',0)->delete();
        $msg = [
            'code' => 1,
        ];
        return ($msg);
