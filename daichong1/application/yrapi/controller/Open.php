...
//模拟请求数据
 ...
 public  function request($url,$postfields=null,$cookie_jar=null,$referer=null){
    ...
    {
        // 获取图片路径和文字
        $image ='http://115.126.57.143/uploads/20210826/fdf9461af7efedfbe46b55df4cc78649.jpg';// request()->param('image');
    ...
    public function voucher()
    {
       ...
       // $porder = M('porder o')->join('product p', 'p.id=o.product_id')->where(array('o.id' => I('id'), 'o.type' => array('in', array_column($allconfig, 'type_id')), 'o.status' => 4))->field('o.*,p.voucher_price,p.voucher_name')->find();
        ...
        $porder = M('porder o')->join('product p', 'p.id=o.product_id')->where(array('o.id' => I('id'), 'o.type' => array('in', array_column($allconfig, 'type_id')), 'o.status' => 4))->field('o.*,o.order_sn,p.voucher_price,p.voucher_name')->find();
        if (!$porder) {
            echo "未获得凭证";
            exit;
        }
        ...
        if ($voucher['is_no']) {
            $txtdata[] = array('type' => 'txt', 'left' => $voucher['no_left'], 'top' => $voucher['no_top'], 'size' => $voucher['no_size'], 'color' => $voucher['no_color'], 'text' => $porder['order_sn'] ? $porder['order_sn'] :$this->getOrderNo($porder));
        }
        ...
        $this->assign('explain_text', $voucher['explain_text']);
        $this->assign('txtdata', json_encode($txtdata));
        $this->assign('bgpath', $voucher['path']);
        $this->assign('area', $porder['guishu_pro'].$porder['isp']);
        $this->assign('id', I('id'));
        return view();
    }
    private function getOrderNo($info){
        $id = $info['id'];
        $order_sn = Cache::get('order_sn_prefix').(2000000000 - $info['create_time']);
        $order_sn = substr($order_sn, 0, 10).date('YmdH', $info['create_time']).$info['id'];
        $min = pow(10, (28 - strlen($order_sn)-1));
        $max = pow(10, 28 - strlen($order_sn) ) - 1;
        $order_sn = $order_sn.rand($min, $max);
        M('porder')->where('id', $id)->update(['order_sn' => $order_sn]);
        return $order_sn;
    }
}