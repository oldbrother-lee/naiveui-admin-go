...
class Index extends Home
{
    ...
    public function recharge()
    {
        ...
        if ($res['errno'] != 0) {
            $data = M('daichong_orders')->where('yr_order_id',$out_trade_num)->find();
            $this->setordertype($data, 8, '失败');
            return ;
            // return djson($res['errno'], $res['errmsg'], $res['data']);
        }
        // echo("222");
        $aid = $res['data'];
        Createlog::porderLog($aid, '创建订单参数'.json_encode(array($mobile, $product_id, array('prov' => $area, 'city' => $city, 'ytype' => $ytype, 'id_card_no' => $id_card_no, 'param1' => $param1, 'param2' => $param2, 'param3' => $param3), $this->customer['id'], Client::CLIENT_API, '', $out_trade_num, $amount, $price), JSON_UNESCAPED_UNICODE));
