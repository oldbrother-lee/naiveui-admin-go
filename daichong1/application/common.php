...
function dc_action($token, $action, $user, $jsonStr)
{
   ...
   $timestamp = time();
    ...
    if($action == "get"){
        ...
        //子账户
        if($user['child'] && strstr($user['child'], '_')){
            $data['sub_username'] = $user['child'];
        }
...
//$user = user 对象
//$jsonStr = 请求参数的 data 数据
function dc_action1($token, $action, $user, $jsonStr)
{
...
{
    if (empty($id)) {
        $user = M('daichong_user_xitu')->where(['id' => 1])->find();
    } else {
...
function get_dc_token()
{
    ...
    if (!empty($res_data['access_token'])) {
        $user_dc = M('daichong_user_xitu')->where(['user' => $user])->find();
        if ($user_dc) {
