...
// 		var_dump($data);exit();
	...
	public static function subApi($porder_id)
	{
		...
		if (!$porder) {
		    Createlog::porderLog($porder_id,'订单无需提交接口充值');
			return rjson(1, '订单无需提交接口充值');
		}
		Rechargeapi::recharge($porder['id']);
		Createlog::porderLog($porder_id,'提交接口工作完成');
	...
	public static function applyCancelOrder2($ids, $operator)
    {
        $porders = M('porder')->where(array('id' => array('in', $ids)))->select();
