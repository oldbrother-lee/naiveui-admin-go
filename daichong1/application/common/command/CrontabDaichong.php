...
class CrontabDaichong extends Command
{
    protected function configure()
    {
        $this->setName('Test')->setDescription('代充测试');
    }
    ...
    {
        $this->post_porder(18939047958, 1088535117297782987, 50);
    }
    ...
    protected function shangbao(){
        $order = M('daichong_order')->where(['sb_type' => 0])->order('id desc')->limit(80)->select();
...
//Use of undefined constant upload - assumed 'upload' (this will throw an Error in a future version of PHP)"
    ...
    public function post_porder($mobile, $out_trade_num, $denom)
    {
        ...
        $guishu = Product::Ispzhan($mobile, 3, $guishu);
		$map['p.isp'] = array('like', '%' . $guishu['data']['isp'] . '%');
		if ($guishu['errno'] != 0) {
			return rjson($guishu['errno'], $guishu['errmsg']);
		}
        $product = M('product')->where(['isp' => $guishu['data']['isp'],'voucher_name' => $denom, 'added' => 1, 'is_del' => 0])->find();
        dump($product);
        echo PHP_EOL;
        dump(['isp' => $guishu['data']['isp'],'voucher_name' => $denom, 'added' => 1, 'is_del' => 0]);
        $data = [
            "userid" => 3,
            "mobile" => $mobile,
        ...
        $resp = $this->http_get('http://115.126.57.143/yrapi.php/index/recharge', $data);
        echo json_encode($data).PHP_EOL;
        dump($resp);
        if(!M('porder')->where(['out_trade_num' => $out_trade_num])->value('id')){
            db('daichong_order')->where(['order_id' => $out_trade_num])->update(['uploadTime' => time(), 'status' => 9, 'type' => 1]);
            $this->sert($out_trade_num, 2);
    ...
    private function http_get($url, $param)
    {
        ...
        if (intval($aStatus["http_code"]) == 200) {
           ...
           return ['errno' => 1, 'errmsg' => '失败', 'ccc' => ['url' => $url, 'param' => json_encode($param)]];
        }
    }
