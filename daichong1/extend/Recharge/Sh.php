...
class Sh
{
     ...
     *      参数名称	类型	必填	备注
            szAgentId	Interger	是	客户ID
            szOrderId	String	是	充值请求时提交的订单号
            szVerifyString	String	是	签名
            szFormat	String	否	返回格式:json(默认)或xml
            1.4.2. 订单查询加密参数（固定）
                array(1) {
                [0] => array(10) {
                ["id"] => int(1)
    ...
    public function selfcheck($order){
        ...
        $sign = md5(urldecode($sign_str));
        $data['szVerifyString'] =$sign;
        $res =  $this->http_post('http://121.40.227.228:80/api/old/queryorder',$data);


        if($res['data']['nRtn']=='5011'){
            Createlog::porderLog($order['id'], "订单自检接口||返回:充值中-|".json_encode($res));
        }elseif($res['data']['nRtn']=='5012'){
