...
class Yuanren
{
    ...
    {
        $data = $_POST;
        $porder_id = M('porder')->where('api_order_number', I('out_trade_num'))->value('id');
        if($porder_id){
            Createlog::porderLog($porder_id, "回调信息返回(1)|".json_encode($data, JSON_UNESCAPED_UNICODE));   
        }
        ...
        if ($state == 1) {
            $resp = PorderModel::rechargeSusApi('yuanren', I('out_trade_num'), $_POST, I('remark'), I('charge_kami'));
